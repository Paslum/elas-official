...
const createNoteWithEmptySections = async (req, res, next) => {
    const { uid, title, course_id } = req.body;
...
const createNote = async (req, res, next) => {
    ...
    const { title, course_id, sections, widgets } = req.body;

    try {
        // Input validation
        if (!uid || !title || !course_id || !sections || !widgets) {
...
const deleteNote = async (req, res, next) => {
    ...
    try {
        ...
        const user = await userModel.findById(note.uid);
        const course = await courseModel.findById(note.course);
